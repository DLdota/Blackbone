...
NTSTATUS BBSafeAllocateString( OUT PUNICODE_STRING result, IN USHORT size )
{
    ...
    result->Buffer = ExAllocatePool2(POOL_FLAG_PAGED, size, BB_POOL_TAG );
    result->Length = 0;
...
NTSTATUS BBSafeInitString( OUT PUNICODE_STRING result, IN PUNICODE_STRING source )
{
    ...
    result->Buffer = ExAllocatePool2(POOL_FLAG_PAGED, source->MaximumLength, BB_POOL_TAG );
...
ULONG GenCall32V( IN PUCHAR pBuf, IN PVOID pFn, IN INT argc, IN va_list vl )
{
    ...
    PULONG pArgBuf = ExAllocatePool2(POOL_FLAG_PAGED, argc * sizeof( ULONG ), BB_POOL_TAG );
